...
class PrototypicalNetwork:
    ...
    def training(self, train_episodes, val_episodes):
        ...
        for epoch in range(self.meta_epochs):
            ...
            losses, accuracies, precisions, recalls, f1s, best_f1, patience = self.proto_model(
                train_episodes, self.updates, val_episodes=val_episodes, best_f1=best_f1, model_path=model_path,
                patience=patience, stopping_threshold=self.stopping_threshold)
            ...
            losses, accuracies, precisions, recalls, f1s, _, _ = self.proto_model(val_episodes, self.updates, testing=True)
    ...
    def proto_analyzer(self, episodes, k, type):
        proto_norms, proto_diffs = self.proto_model.proto_analyzer(episodes[:1000])
        #plt.figure(figsize=(4, 4))
        # ax = plt.subplot(121)
        # bx = plt.subplot(122)
        # ax.set_title("l2-norms", fontsize=14)
        # bx.set_title("l2-norm diffs", fontsize=14)
        # plt.hist(proto_diffs, bins=40, density=True, facecolor='red', edgecolor='black', alpha=0.8)
        # # bx.hist(proto_diffs, bins=40, density=True, facecolor='green', edgecolor='black', alpha=0.8)
        # # proto_diffs = np.array(proto_diffs)
        # print(np.min(proto_diffs), np.max(proto_diffs), np.std(proto_diffs), np.mean(proto_diffs))
        # plt.savefig("proto_diffs.png", bbox_inches='tight')
        # plt.close()
        np.save("wsd_norm_diff.npy", np.array(proto_diffs))