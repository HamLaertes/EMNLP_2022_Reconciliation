...
word_raw_vectors = torch.cat(word_raw_vectors, dim=0).numpy()
...
word_trained_vectors = torch.cat(word_trained_vectors, dim=0).numpy()

print("Successful Map Words to Vectors..Start Drawing Figures and Output...")
word_frequency = np.array(word_frequency)
word_diff_vectors = word_raw_vectors - word_trained_vectors
word_diff_vectors = np.linalg.norm(word_diff_vectors, axis=-1)
...
plt.colorbar()
plt.savefig("bert_norm_frequency_compared_{}.jpg".format(few_setting))
plt.close()