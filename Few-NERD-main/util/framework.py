...
class FewShotNERFramework:
    ...
    def train(self,
        ...
        parameters_to_optimize = [
            {'params': [p for n, p in parameters_to_optimize 
                if (not any(nd in n for nd in no_decay)) and 'domain' not in n], 'weight_decay': 0.01},
            {'params': [p for n, p in parameters_to_optimize
                if any(nd in n for nd in no_decay) and 'domain' not in n ], 'weight_decay': 0.0},
            {'params': [p for n, p in parameters_to_optimize
                if (not any(nd in n for nd in no_decay)) and 'domain' in n], 'weight_decay': 0.01, 'lr': learning_rate * 1},
            {'params': [p for n, p in parameters_to_optimize
                if any(nd in n for nd in no_decay) and 'domain' in n], 'weight_decay': 0.0, 'lr': learning_rate * 1},
        ]
        if use_sgd_for_bert:
        ...
        while it + 1 < train_iter:
            for _, (support, query) in enumerate(self.train_data_loader):
                ...
                logits, pred, log_p = model(support, query)
                ...
                if (it + 1) % 100 == 0 or (it + 1) % val_step == 0:
                    ...
                    sys.stdout.write('step: {0:4} | loss: {1:2.6f} | [ENTITY] precision: {2:3.4f}, recall: {3:3.4f}, f1: {4:3.4f}'\
                        .format(it + 1, iter_loss/ iter_sample, precision, recall, f1) + '\n')
                sys.stdout.flush()
            ...
            while it + 1 < eval_iter:
                for _, (support, query) in enumerate(eval_dataset):
                    if torch.cuda.is_available():
                        ...
                        label = torch.cat(query['label'], 0)
                        label = label.cuda()
                    logits, pred, _ = model(support, query)
                    if self.viterbi:
    ...
    def norm_analyze(self,
        ...
        cos_sim = []
        proto_dict = {}
        for it, (support, query) in tqdm(enumerate(self.train_data_loader)):
            if torch.cuda.is_available():
                for k in support:
                    if k != 'label' and k != 'sentence_num':
                        support[k] = support[k].cuda()
                ...
                label = torch.cat(query['label'], 0)
                label = label.cuda()
            norm_diff_, norm_proto_, proto_dict_  = model.norm_analyze(support, query, label)
            norm_diff += norm_diff_
            norm_proto += norm_proto_.tolist()
            for k, v in proto_dict_.items():
                tmp = proto_dict.get(k, [])
                tmp += v
                proto_dict[k] = tmp
            if it == 20:
                break
        # data = []
        # label = []
        # for i, (k, v) in enumerate(proto_dict.items()):
        #     label += [i for _ in range(len(v))]
        #     data += [F.normalize(d, dim=0).tolist() for d in v]
        # data = np.array(data)
        # label = np.array(label)
        # np.save("p_raw_data.npy", data)
        # np.save("l_raw_data.npy", label)
        # wsd = np.load("wsd_norm_diff.npy")
        # plt.figure(figsize=(10, 4))
        # ax = plt.subplot(121)
        # bx = plt.subplot(122)
        # plt.subplots_adjust(wspace=0.4)
        # ax.set_title("Few-NERD", fontsize=14)
        # bx.set_title("Word Sense Disambiguate", fontsize=14)
        # ax.tick_params(axis='both', which='major', labelsize=14)
        # bx.tick_params(axis='both', which='major', labelsize=14)
        # ax.hist(norm_diff, bins=40, density=True, facecolor='green', edgecolor='black', alpha=0.8)
        # bx.hist(wsd, bins=40, density=True, facecolor='red', edgecolor='black', alpha=0.8)
        # plt.savefig("norm_diff.png", bbox_inches='tight')
        # norm_diff = np.array(norm_diff)
        # print(np.min(norm_diff), np.max(norm_diff), np.std(norm_diff), np.mean(norm_diff))
        # plt.close()
        #
        # print(torch.var(torch.tensor(norm)).item(), torch.var(torch.tensor(norm)).item() / torch.tensor(norm).mean().item())
        # plt.figure(figsize=(10, 10))
        # plt.hist(norm, bins=40, density=True, facecolor='green', edgecolor='black', alpha=0.7)
        # plt.savefig("nerd_norm.jpg")
        # plt.close()
        ...
        # print(torch.var(torch.tensor(norm_proto)).item(), torch.var(torch.tensor(norm_proto)).item() / torch.tensor(norm_proto).mean().item())
        # plt.figure(figsize=(8, 4))
        # plt.hist(norm_proto, bins=40, density=True, facecolor='red', edgecolor='black', alpha=0.7)
        # plt.savefig("nerd_norm_proto_trained.jpg")
        # np.save("norm_proto_trained.npy", np.array(norm_proto))

        # print(torch.var(torch.tensor(cos_sim)).item(),
        #       torch.var(torch.tensor(cos_sim)).item() / torch.tensor(cos_sim).mean().item())
        # plt.figure(figsize=(10, 10))
        # plt.hist(cos_sim, bins=40, facecolor='green', edgecolor='black', alpha=0.7)
        # plt.savefig("cos_sim.jpg")
        # plt.close()
        ...
        for k, v in proto_dict.items():
            ...
            v_norm = torch.linalg.norm(v, dim=-1).tolist()
            max_.append(max(v_norm))
            min_.append(min(v_norm))
            mean_.append(sum(v_norm) / len(v_norm))
            i += 1
            if i == n:
                break
        np.save("proto_norm_c.npy", np.array(x))
        np.save("max_proto_norm.npy", np.array(max_))
        np.save("min_proto_norm.npy", np.array(min_))
        np.save("avg_proto_norm.npy", np.array(mean_))
